...
import { setHotTickets, setUpcomingEvents } from './redux/actions/actions';
import './index.scss';
import App from './App';
...
axios.get('https://picsum.photos/v2/list?page=4&limit=6')
  .then(response => store.dispatch(setHotTickets(response.data)))
  .then(() => {
    axios.get('https://picsum.photos/v2/list?page=10&limit=3')
      .then(response => store.dispatch(setUpcomingEvents(response.data)))
      .then(() =>
        ReactDOM.render(
